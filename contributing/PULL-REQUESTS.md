...
* [Reviewing Pull Requests](#reviewing-pull-requests)
  ...
  * [Accept that there are different opinions about what belongs](#accept-that-there-are-different-opinions-about-what-belongs)
...
Fork the project [on GitHub](https://github.com/IBM/sdl2-gamecontroller) and clone your fork
locally.

```text
$ git clone git@github.com:username/sdl2-gamecontroller.git
$ cd sdl2-gamecontroller
$ git remote add upstream git@github.com:IBM/sdl2-gamecontroller.git
$ git fetch upstream
```
...
lint you files before checing them in. `npm run lint`
...
If you are new to contributing to sdl2-gamecontroller, please try to do your best at
...
Pull Request against (probably `main`), you should see a commit with
...
2. Does this change make the code better, even if only incrementally?
...
reviewer landing the Pull Request but they can also be an
...
a particular change that makes the code better, but the individual might just
not want to have anything to do with this repo ever again. The goal is not just
having good code.
...
Any Collaborator (any GitHub user with commit rights in the
...
### Accept that there are different opinions about what belongs
...
make the code better. These may or may not make sense to add, but as with all
...
We try to optimize for speed of execution. If a particular change
can be shown to make some part of the code faster, it's quite likely to be
...
That said, performance is not the only factor to consider. We favor of not breaking existing code in the ecosystem, and not
...
to make sure that it works on the platforms that the code
supports. This is currently via Git Actions.
...
when each individual commit lands on the main branch.
